...
- 注文系のエンドポイント（3〜5）は `/api/customers/{customerId}/orders` を気点とし、顧客配下のリソースとして設計しました。ただし注文登録（3）は `orderId` を主キーとする独立リソースのため `/api/orders` に POST する設計とした（作成時にボディへ `customerId` を含める）。
- 404 は「パスで指定した顧客が存在しない」場合に使用し、400 は「リクエスト自体の形式・値が不正」な場合に使い分けています。
- 409 は必須項目には含まれない `HTTPステータスコードの目安` 表にはないが、ID重複という現実的な異常系のため追加しました。
